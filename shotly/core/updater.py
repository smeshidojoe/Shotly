...
Поток:
  ...
  3. restart_to_update()   — достаём новый exe из архива и запускаем его с флагом
     --apply-update <старый exe>; сразу после этого приложение закрывается. Новый
     exe дожидается, пока старый освободится, подменяет его собой и запускает.
     Так можно заменить и работающий exe, который во время работы заблокирован.
...
def restart_to_update():
    """Готовит новый exe и запускает подмену. После True вызывающий обязан сразу
    завершить приложение, чтобы освободить свой exe-файл."""
...
def relaunch_app():
    ...
    заново. Вызывающий обязан сразу завершить приложение. True — помощник запущен."""
