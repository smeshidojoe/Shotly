...
class Toast(QWidget):
    ...
    def paintEvent(self, e):
        ...
        if self._subtext:
            ...
            p.drawText(QRectF(x, y, self.width() - x - self._pad, fm.height()),
            ...
            p.drawText(QRectF(x, y + fm.height() + theme.s(2),
                              self.width() - x - self._pad, sfm.height()),
                       Qt.AlignLeft | Qt.AlignVCenter, self._subtext)
        else:
            p.setFont(self._font)
            p.setPen(QPen(theme.color("text")))
            p.drawText(QRectF(x, 0, self.width() - x - self._pad, self.height()),
                       Qt.AlignLeft | Qt.AlignVCenter, self._text)
        p.end()
...
def show(text, subtext="", icon_name="check", open_path="", on_click=None):
    return Toast(text, subtext, icon_name, open_path, on_click).show_at_corner()
